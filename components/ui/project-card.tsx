...
export function ProjectCard({
  ...
  return (
    <Card className="group overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 hover:border-primary/50 h-full flex flex-col">
      {imageUrl ? (
        ...
        >
          ...
          />
          <div className={`absolute -top-12 -right-12 w-32 h-32 rounded-full bg-gradient-to-br ${theme.gradient} opacity-40 blur-2xl`} />
          <div className={`absolute -bottom-8 -left-8 w-24 h-24 rounded-full bg-gradient-to-br ${theme.gradient} opacity-30 blur-xl`} />
        </div>
      )}
        ...
        <CardTitle className="flex items-center justify-between min-h-[2.25rem]">
          ...
          <div className="flex items-center gap-3">
            {githubRepos?.map((repo) => {
              ...
              const translatedLabel = typeof translated === 'string' ? translated : repo.label;
